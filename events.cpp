...
template<class T>
shared_ptr<T> events::new_watcher(function<void(events*)> callback) {
...
void events::onSignal(int signal, function<void(events*)> callback) {
...
void events::onTimer(ev_tstamp after, ev_tstamp repeat, function<void(events*)> callback) {
    shared_ptr<event_timer_watcher> e_spec = new_watcher<event_timer_watcher>(callback);
...
void events::onRead(int fd, function<void(events*)> callback) {
    shared_ptr<event_io_watcher> e_spec = new_watcher<event_io_watcher>(callback);
...
void events::signal_callback(struct ev_loop* loop, ev_signal* signal, int event) {
    ...
    auto callback = static_cast <function<void(events*)>> (e_spec->callback);
    callback(e_spec->self);
}
